...
elseif ($_GET['m'] == 'daftar') {
    ...
    if ($count > 0) {
        ...
        // Validasi input
        if (empty($no_daftar) || empty($alamat) || empty($program) || empty($no_hp) || empty($nama) || empty($email)) {
            echo "<script>window.alert('Semua data harus diisi. Gagal input data.');
            window.location=('index.php#daftar')</script>";
            exit();
        }
        // Query untuk memeriksa apakah email sudah ada di database
    
        $query = "INSERT INTO daftar (no_daftar, alamat, program, no_hp, nama, email, id_sesi, tgl_daftar, waktu,password,show_pass,status_data) 
        VALUES ('$no_daftar', '$alamat', '$program', '$no_hp', '$nama', '$email', '$sesi', '$tgl_daftar','$waktu','$password','$_POST[password]','proses')";
        mysqli_query($koneksi, $query);
        $id_daftar = mysqli_insert_id($koneksi);
        // Eksekusi query

    $log = mysqli_query($koneksi, "SELECT * FROM daftar WHERE email='$email' AND password='$password'");
    $cek = mysqli_num_rows($log);
    if($cek > 0){
        session_start();
        $d = mysqli_fetch_assoc($log);
        $_SESSION['id_daftar'] = $d['id_daftar'];
        $_SESSION['nama'] = $d['nama'];
        $_SESSION['email'] = $d['email'];
        $_SESSION['status'] = "siswa";
        echo "<script>
        window.alert('Pendafraran berhasil silahkan lengkapi data');
        window.location.href = 'siswa/index.php?aksi=home';
      </script>";
    }else{
      echo "<script>
...
elseif ($_GET['aksi'] == 'login') {
 ...
 if($cek > 0){
     ...
     echo "<script>window.alert('Email atau password salah'); window.location=('index.php#daftars')</script>";
